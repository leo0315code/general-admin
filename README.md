...
- 仪表盘：统计卡片 + 最近文章
- 用户管理：CRUD、角色分配、重置密码、**账号启停（停用不可登录）**、**登录痕迹（最后登录时间/IP）**、**首登强制改密**、**最后一个 admin 保护（不可删除/停用/降级）**、软删除、**回收站（还原/彻底删除）**、**Excel 导入/导出**（导入：中文表头、姓名/邮箱查重含软删、失败行跳过不中断、密码留空自动生成随机强密码；导出跟随当前筛选条件）
...
- **数据范围授权（Policy）**：`PostPolicy`（`App\Policies`，Laravel 自动发现）——admin 或作者本人可编辑/删除文章，其它用户对他人文章一律 403
- **最后一个 admin 保护**：唯一启用的 admin 不可被删除、停用或移除 admin 角色（`UserController::isLastActiveAdmin`），避免系统进入无人可管理状态
...
- **账号启停**：`users.status=0` 的账号登录被拒（提示"账号已被停用"）；列表/编辑页可一键启停（不能停用自己）
- **登录痕迹**：登录成功记录 `last_login_at` / `last_login_ip`，列表与编辑页展示
- **首登强制改密**：导入（随机密码）或管理员重置密码后 `must_change_password=1`，下次登录强制跳转 `/{prefix}/password-setup` 设置新密码，完成后放行
