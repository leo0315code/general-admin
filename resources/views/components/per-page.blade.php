...
{{-- 每页条数（原生 JS：onchange 跳转，保留现有 query） --}}
<div class="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
    <span class="select-none">每页</span>
    <select
        class="input !w-auto !px-2.5 !py-1.5 text-sm"
        aria-label="每页条数"
        onchange="const u=new URL(window.location.href);u.searchParams.set('{{ $name }}',this.value);u.searchParams.set('page','1');window.location.href=u.toString();"
    >
        @foreach ($list as $opt)
